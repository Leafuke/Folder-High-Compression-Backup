...
string rname2[20],Bpath,command,yasuo;
bool prebf,ontop,choice;
HWND hwnd;
struct File {
    string name;
    time_t modifiedTime;
    ...
    if (!file.is_open()) {
        ...
        if (newFile.is_open()) {
        	...
        	for(int i=1;i<=summ;++i)
        		newFile << Gpath2[i] << endl;
        	newFile << '*' << endl;
            newFile << "Backup Storage Folder Path:" << Bpath << endl;
			string keyPath = "Software\\7-Zip"; 
			string valueName = "Path";
            newFile << "Compressed software path:" << GetRegistryValue(keyPath, valueName)+"7z.exe" << endl;
    	...
    	for(int i=0;i<=summ;++i)
    	{
		    ...
		    {
		        string alias;
		        cout << "Please enter an alias for folder " << folderName;
		        cin >> alias;
				newFile << folderName << " " << alias << std::endl;
		    }
		    newFile << "$" << endl;
		}
	    newFile<< "*" << endl;
